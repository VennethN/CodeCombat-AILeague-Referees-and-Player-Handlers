...
(class IceAndIronPlayer extends Component {
    ...
    _special(specialName, x, y) {
        ...
        if (specialName == 'teleport') {
            ...
            const guards = this.world.thangs.filter(t => t.exists && !t.dead && t.type == 'guard');
            for (let m of this._ref.monsters) {
                if (m.nonferrous) continue;
                if (m.distanceSquared(pos) < rangeSq) {
                    let teleportPos = m.pos.copy().add(dir);
                    if (m.teleportPos) {
                        teleportPos = m.teleportPos.copy().add(teleportPos).multiply(0.5);
                    }
                    m.teleportPos = teleportPos;
                    m.stunTime = this.world.age + params.stunDuration;
                    m.aggro = null;
                    guards.forEach((g) => {
                        if (g.target && g.target == m) {
                            g.setTarget(null);
                            g.setAction('idle');
                            g.unblock();
                        }
    ...
    esper_findMonsters() {
        return this._ref.monsters.filter((m) => m.exists && !m.dead && !m.markedDead);
    }
    ...
    spawnGuard(place) {
        ...
        guard.healthReplenishRate = params.healthReplenishRate || 0;
        ...
        guard._takeDamage = guard.takeDamage;
        const rangedAttackRatio = params.rangedAttackRatio || 1;
        guard.takeDamage = (damage, attacker, momentum=null) => {
            if (attacker.ranged) {
                damage *= rangedAttackRatio;
            }
            guard._takeDamage(damage, attacker, momentum);
        };
        guard.initialize();
        return guard;
    ...
    setupGuardAPI(guard, place, params) {
        ...
        guard.moveTowards = function(x, y) {
            if (this.color == 'blue') {
                x = 120 - x;
            }
            return this.move(new Vector(x, y));
